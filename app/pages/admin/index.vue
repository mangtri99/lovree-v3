...
const cards = computed(() => [
  {
    label: "Undangan",
    value: (stats.value as any)?.invitations ?? 0,
    icon: "i-lucide-mail",
  },
  {
    label: "Published",
    value: (stats.value as any)?.published ?? 0,
    icon: "i-lucide-globe",
  },
  {
    label: "Draft",
    value: (stats.value as any)?.drafts ?? 0,
    icon: "i-lucide-pencil",
  },
  {
    label: "Tamu",
    value: (stats.value as any)?.guests ?? 0,
    icon: "i-lucide-users",
  },
  {
    label: "RSVP",
    value: (stats.value as any)?.rsvps ?? 0,
    ...
    <template #body>
      <div class="space-y-6">
        ...
        <UCard>
          ...
          <div v-if="!recent.length" class="text-sm text-gray-500">
            Belum ada undangan.
          </div>
          <ul v-else class="space-y-2">
            ...
            >
              <span>{{ inv.slug }}</span>
              <UBadge
                :color="inv.status === 'published' ? 'success' : 'neutral'"
                variant="subtle"
                :label="inv.status"
              />
              <UButton
