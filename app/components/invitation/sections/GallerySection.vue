...
type ImageItem = { type: 'image'; image: { mediaId: string; url: string } }
...
const renderable = computed(() => (props.content.items ?? []).filter((it) =>
  it.type === 'image' ? !!it.image?.url : isValidYoutube(it.videoId)))
</script>
<template>
  <section class="px-2 py-12">
    <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
      <template v-for="(item, i) in renderable" :key="i">
        ...
        <img v-else :src="item.image.url" alt="" class="h-full w-full object-cover" loading="lazy" />
      </template>
    </div>
  </section>
</template>
