...
<template>
  <section class="px-6 py-14" style="background: var(--color-bg); color: var(--color-text)">
    ...
    <form v-else class="mx-auto mt-4 max-w-md space-y-3" @submit.prevent="submit">
      <input v-model="name" placeholder="Nama" class="w-full p-2" style="border: 1px solid var(--color-accent); border-radius: var(--radius-md)" />
      <select v-model="attendance" class="w-full p-2" style="border: 1px solid var(--color-accent); border-radius: var(--radius-md)">
        <option value="yes">Hadir</option>
        <option value="no">Tidak Hadir</option>
        <option value="maybe">Mungkin</option>
      </select>
      <textarea v-model="message" placeholder="Ucapan & Doa" class="w-full p-2" style="border: 1px solid var(--color-accent); border-radius: var(--radius-md)" />
      <p v-if="error" class="text-sm text-red-600">{{ error }}</p>
      <button type="submit" :disabled="submitting" class="w-full rounded p-2" style="background: var(--color-primary); color: var(--color-bg)">{{ submitting ? 'Mengirim…' : 'Kirim' }}</button>
    </form>
  </section>
</template>
